...
def withdraw(opt_app_id, opt_asa_id):
    ...
    try:
        ...
        sender_wallet = withdraw_payload['sender_waller']
        fee_address = withdraw_payload['fee_address']
        opt_amt = withdraw_payload['opt_amt']
...
def compute_algo_withdraw_amt_from_opt(opt_app_id, opt_asa_id, opt_amt):
    ...
    try:
        return_value = transactions.Controller.withdraw.compute_algo_withdraw_amt_from_opt(opt_app_id, opt_asa_id, opt_amt)
...
def withdraw_from_custodial_wallets(opt_app_id):
    ...
    if wallet_info == "True":
        try:
            txn_object = transactions.Controller.withdraw.withdraw_from_custodial_wallets(sender_wallet, opt_app_id, withdraw_amt)
            return jsonify(txn_object), 200
        except Exception as error:
            return jsonify({'message': f"Server Error! {error}"}), 500
    elif wallet_info == "False":
